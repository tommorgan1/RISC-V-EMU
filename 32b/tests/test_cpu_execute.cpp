...
// faults (illegal instruction, memory fault, etc.) are never silently swallowed.
...
TEST_CASE("Execute SYS: ECALL", "[cpu][SYS]")
{
    ...
    cpu.load(prog, 0x80000000);
    try { cpu.run(); FAIL("Expected ECALL throw"); }
    catch (const std::runtime_error& e) { REQUIRE(std::string(e.what()) == "ECALL"); }
}
...
TEST_CASE("Execute SYS: EBREAK", "[cpu][SYS]")
{
    ...
    cpu.load(prog, 0x80000000);
    try { cpu.run(); FAIL("Expected EBREAK throw"); }
    catch (const std::runtime_error& e) { REQUIRE(std::string(e.what()) == "EBREAK"); }
}
...
TEST_CASE("Execute SYS: CSR read-modify-write sequence", "[cpu][SYS]")
{
    ...
    //   csrrwi x2, 0x300, 5   → x2 = old(0),  mstatus = 5    (0x300 2D173 → bytes below)
    //   csrrsi x3, 0x300, 2   → x3 = old(5),  mstatus = 7    (5|2)
    //   csrrci x4, 0x300, 1   → x4 = old(7),  mstatus = 6    (7&~1)
    std::vector<uint8_t> prog = {
