...
CPU::CPU (uint32_t base, size_t memSize) : _memory (base, memSize), _pc (base)
{
}
...
void CPU::execute_R (const InstructionField& field)
{
	...
	switch ((field.funct3.value () << 8) | field.funct7.value ())
	{
		...
		case (0x5 << 8) | 0x00:
			...
			break;
		case (0x5 << 8) | 0x20:
			result = static_cast<uint32_t> (static_cast<int32_t> (rs1) >> (rs2 & 0x1F));
			break;
		case (0x6 << 8) | 0x00:
			result = rs1 | rs2;
			break;
...
void CPU::execute_I (const InstructionField& field)
{
	...
	{
		case 0x0:
			result = static_cast<uint32_t> (static_cast<int32_t> (rs1) + imm);
			break;
		case 0x2:
		...
		case 0x3:
			...
			break;
		case 0x4:
			result = rs1 ^ static_cast<uint32_t> (imm);
			break;
		case 0x6:
			result = rs1 | static_cast<uint32_t> (imm);
			break;
...
{
	uint32_t addr   = static_cast<uint32_t> (static_cast<int32_t> (readReg (field.rs1.value ())) +
	                                         field.imm.value ());
	uint32_t result = 0;
...
void CPU::execute_S (const InstructionField& field)
{
	...
	{
		case 0x0:
			f = _memory.write8 (addr, static_cast<uint8_t> (src));
			break;  // SB
		case 0x1:
			f = _memory.write16 (addr, static_cast<uint16_t> (src));
			break;  // SH
		case 0x2:
			f = _memory.write32 (addr, src);
			break;  // SW
		default:
			throw std::runtime_error ("Unknown funct3 for S-type at PC: " + hex32 (_pc - 4));
	}

	if (f != memFault::none) throw std::runtime_error ("Store memory fault at " + hex32 (addr));
}
...
void CPU::execute_B (const InstructionField& field, uint32_t current_pc)
{
	...
	if (taken) _pc = static_cast<uint32_t> (static_cast<int32_t> (current_pc) + imm);
}
...
void CPU::execute_U (const InstructionField& field, uint32_t current_pc)
{
	...
	{
		case 0x37:
			writeReg (field.rd.value (), imm);
			break;  // LUI
		case 0x17:
			writeReg (field.rd.value (), current_pc + imm);
			break;  // AUIPC
		default:
...
{
	uint32_t target = static_cast<uint32_t> (static_cast<int32_t> (readReg (field.rs1.value ())) +
	                                         field.imm.value ());
	writeReg (field.rd.value (), (current_pc + 4));
	_pc = (target & ~1u);
}
